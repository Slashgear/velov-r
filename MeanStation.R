...
boxplot(sat,horizontal=T,main="Variance de la saturation par station")
dev.off()
...
a <- ggmap(mapImageData1, extent = "panel")
a <- a + geom_point(data=d, aes(x = lat,y = lon), colour=cl5$cluster)
...
plot(scale(stationBonus$ALTITUDE),stationBonus$SATURATION,main="Variance de la saturation par rapport à l'altitude")
...
plot(sat_alt1[,1],sat_alt1[,2],main="Variance de la saturation par rapport à l'altitude Groupe 1")
...
plot(sat_alt2[,1],sat_alt2[,2],main="Variance de la saturation par rapport à l'altitude Groupe 2")
